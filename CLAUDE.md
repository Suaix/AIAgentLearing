...
Step 2 — 核心机制精讲（结构化总结，v2.5 增强版）
  ...
  **B. 深度展开**（必做，每节 1-2 个）：
    选 1-2 个最核心的概念做 3-5 分钟阅读量的深入解释
    示例：Pooling 策略、训练目标（对比学习）、余弦 vs 欧氏的数学本质
    目标：关键概念不只"知道"，而是"理解为什么"
  
  **C. 补充概念**（必做，每节 3-4 个）：
    列出 Step 0 代码没体现但属于本主题的重要概念
    每个 2-3 句话覆盖，建立完整知识边界
    示例：Token 截断影响、向量算术、对称 vs 非对称语义
    **引入新工具/库时必须回答两个问题**：
      ① 为什么练习用这个？（学习阶段的选型理由）
      ② 生产环境有哪些替代品？怎么选？（全景图 + 决策树）
    目标：让学习者知道本主题的完整版图，不只是代码触及的部分
...
Step 4 — 费曼检验（轻量版，v2.5 增强）
  学习者用 2-3 句话解释"这段代码做了什么、为什么这样设计"
  Claude 挑一个模糊点追问，不要求长篇复述
  追问范围：代码相关概念 + Step 2 补充概念板块中提到的知识点
  目标：确认理解精准度，同时检验完整知识版图（不只代码触及的部分）
...
- **定位**：该主题的完整知识地图——覆盖代码里有的和代码里没有的，不是代码注释的搬运工
- 必须包含：一句话总结、核心概念（含 Step 2 补充概念）、代码关联（`code/` 目录下的文件路径）、与后续模块的关联
